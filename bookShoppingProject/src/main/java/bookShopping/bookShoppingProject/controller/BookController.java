...
//		List<Book> books = bookDAO.findAll();
	...
	public String openNewBookPage(Model model) {
		...
		model.addAttribute("header","YENİ KİTAB");
		return "new-book";
	}
	
	@PostMapping(path="/books/new-book-process")
	public String saveBook(@Valid @ModelAttribute(name="book")Book book, BindingResult result, @RequestParam(value = "imageFile", required = false) MultipartFile imageFile, Model model) {
		if(result.hasErrors()) {
			return "new-book";
		}
	...
	public String editBook(@PathVariable(name="id")Integer id, Model model) {
		...
		model.addAttribute("book", book);
		model.addAttribute("header","KİTAB REDAKTƏSİ");
		return "new-book";
	}
}
